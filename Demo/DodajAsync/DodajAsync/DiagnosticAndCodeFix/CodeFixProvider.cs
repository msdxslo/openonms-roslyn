...
namespace DiagnosticAndCodeFix
{
    ...
    public class CodeFixProvider : ICodeFixProvider
    {
        ...
        public async Task<IEnumerable<CodeAction>> GetFixesAsync(Document document, TextSpan span, IEnumerable<Diagnostic> diagnostics, CancellationToken cancellationToken)
        {
            ...
            var metoda = (MethodDeclarationSyntax)root.FindNode(diagnosticSpan);

            // Return a code action that will invoke the fix.
            return new[] { CodeAction.Create("Dodaj Async postfix", c => DodajAsync(document, metoda, c)) };
        }

        private async Task<Solution> DodajAsync(Document document, MethodDeclarationSyntax metoda, CancellationToken cancellationToken)
        {
            // Compute new uppercase name.
            var identifierToken = metoda.Identifier;
            var newName = identifierToken.Text + "Async";

            // Get the symbol representing the type to be renamed.
            var semanticModel = await document.GetSemanticModelAsync(cancellationToken);
            var typeSymbol = semanticModel.GetDeclaredSymbol(metoda, cancellationToken);
